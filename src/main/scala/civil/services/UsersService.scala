...
object UsersService {
  ...
  def getUserUnauthenticated(
      id: String
  ): ZIO[UsersService, AppError, OutgoingUserUnauthenticated] =
    ZIO.serviceWithZIO[UsersService](_.getUserUnauthenticated(id))
...
case class UsersServiceLive(
  ...
  override def getUser(
    ...
    } yield outgoingUser.copy(
      iconSrc = Some(userData.userIconSrc),
  ...
  override def getUserUnauthenticated(
    ...
    usersRepository.getUserUnauthenticated(id)
  }
...
object UsersServiceLive {
  val layer: URLayer[UsersRepository with AuthenticationService, UsersService] =
    ZLayer.fromFunction(UsersServiceLive.apply _)
}
