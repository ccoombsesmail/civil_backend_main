...
final case class CommentLikesController(
    commentCivilityService: CommentLikesService
) {
    ...
    Http.collectZIO[Request] {
      case req @ Method.PUT -> !! / "api" / "v1" / "comments" / "likes" =>
        (for {
          ...
          likeGivenResponse <- commentCivilityService
            .addRemoveCommentLikeOrDislike(jwt, jwtType, updateCommentLikes)
      ...
      case req @ Method.PUT -> !! / "api" / "v1" / "comments" / "likes-tribunal" =>
        (for {
          ...
          (jwt, jwtType) = authData
          likeGivenResponse <- commentCivilityService
            .addRemoveTribunalCommentLikeOrDislike(
              jwt,
              jwtType,
              updateCommentLikes
            )
...
object CommentLikesController {
  ...
  val layer: URLayer[CommentLikesService, CommentLikesController] =
    ZLayer.fromFunction(CommentLikesController.apply _)
