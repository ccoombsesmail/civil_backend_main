...
import civil.models.{ErrorInfo, FollowedUserId, OutgoingUser}
import sttp.tapir._
...
object FollowsApi {
  ...
  val newFollowEndpoint: Endpoint[(String, String, FollowedUserId), ErrorInfo, OutgoingUser, Any] =
    baseEndpointAuthenticated.post
      .in("follows")
      .in(jsonBody[FollowedUserId])
      .out(jsonBody[OutgoingUser])

  val deleteFollowEndpoint: Endpoint[(String, String, String), ErrorInfo, OutgoingUser, Any] =
