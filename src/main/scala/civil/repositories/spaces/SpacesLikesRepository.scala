...
case class SpaceLikesRepositoryLive(dataSource: DataSource)
    extends SpaceLikesRepository {
  ...
  override def addRemoveSpaceLikeOrDislike(
    ...
    (for {
      previousLikeState <- run(
        query[SpaceLikes].filter(tl =>
          tl.spaceId == lift(spaceLikeDislikeData.id) && tl.userId == lift(
            userId
          )
        )
      ...
      prevLikeState = previousLikeState.headOption
        .getOrElse(SpaceLikes(spaceLikeDislikeData.id, userId, NeutralState))
        .likeState
        ...
        for {
          _ <- run(
            query[SpaceLikes]
              .insertValue(
                lift(
                  SpaceLikes(
                    spaceLikeDislikeData.id,
                    userId,
                    spaceLikeDislikeData.likeAction
                  )
                )
              )
              .onConflictUpdate(_.spaceId, _.userId)((t, e) =>
          ...
          updatedSpace <- run(
            query[Spaces]
              ...
              .update(space =>
                space.likes -> (space.likes + lift(likeValueToAdd))
              )
              .returning(t => t)
          )
        } yield updatedSpace
      ...
      SpaceLiked(space.id, space.likes, spaceLikeDislikeData.likeAction),
      space
...
object SpaceLikesRepositoryLive {
  ...
  val layer: URLayer[DataSource, SpaceLikesRepository] =
    ZLayer.fromFunction(SpaceLikesRepositoryLive.apply _)
